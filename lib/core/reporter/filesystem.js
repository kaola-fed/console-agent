...
module.exports = class FileSystemReporter extends Reporter {
    ...
    async report(list) {
        ...
        const filename = `metrix-results-${date.getFullYear()}-${date.getMonth()}-${date.getDate()}_${date.getHours()}_${date.getMinutes()}.json`;

        await fs.rwrite(path.join(os.homedir(), 'agentk', this.name, filename), JSON.stringify(json, null, 4));
